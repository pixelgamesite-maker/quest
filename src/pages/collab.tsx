...
const INIT: Form = { communityName: "", xHandle: "", discordServer: "", website: "", spotsRequested: "", description: "" };
...
export default function Collab() {
  ...
  const inputClass = "w-full bg-black border border-zinc-800 text-white px-4 py-3 text-xs font-mono rounded-sm transition-all focus:border-orange-500/40";
    ...
    return (
      <MainLayout>
        <div className="min-h-[calc(100vh-64px)] flex flex-col items-center justify-center px-6 text-center">
          ...
          <h1 className="font-serif text-4xl font-black text-white tracking-widest mb-4">APPLY FOR COLLAB</h1>
          <p className="text-xs text-zinc-500 leading-relaxed mb-10 max-w-sm">
            Sign in with Discord to submit your collab request.<br />
            Your username will be captured automatically.
          </p>
          <a href={getDiscordAuthUrl("collab")}>
            <button className="flex items-center gap-3 px-8 py-4 bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs tracking-[0.3em] rounded-sm transition-all cursor-pointer border-none">
      ...
      <MainLayout>
        <div className="max-w-lg mx-auto px-6 py-24 text-center">
          <div className="bg-zinc-950 border border-green-400/20 rounded-sm p-12">
            ...
            <div className="font-serif text-xl font-black tracking-[0.3em] text-white mb-4">REQUEST SUBMITTED</div>
            <p className="text-xs text-zinc-500 leading-relaxed mb-8">
              We'll review <strong className="text-white">{form.communityName}</strong>'s request and reach out within 48 hours via Discord.
            </p>
    ...
    <MainLayout>
      <div className="max-w-4xl mx-auto px-6 py-16">
        <div className="text-center mb-12">
          <p className="text-[10px] tracking-[0.5em] text-orange-400/70 mb-4">PROJECT COLLAB</p>
          <h1 className="font-serif text-4xl md:text-5xl font-black text-white tracking-widest mb-5">REQUEST COLLAB</h1>
          <p className="text-xs text-zinc-500 leading-relaxed mb-6">
            Request WL spots for your community from Earnity.<br />
          ...
          <div className="inline-flex items-center gap-3 bg-zinc-950 border border-white/5 rounded-full px-4 py-2">
            ...
            <span className="text-xs text-zinc-400">{discordUser?.username}</span>
            <button onClick={signOutDiscord} className="text-[9px] text-zinc-700 hover:text-zinc-500 tracking-widest cursor-pointer bg-transparent border-none">
              SIGN OUT
            </button>
          </div>
        </div>
        ...
        <div className="bg-zinc-950 border border-white/5 rounded-sm p-8 max-w-2xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
            <div className="md:col-span-2">
              ...
              <input className={inputClass} placeholder="e.g. Pixel Realms" value={form.communityName} onChange={set("communityName")} />
            </div>
            <div>
              <label className={labelClass}>X (TWITTER) HANDLE</label>
              <input className={inputClass} placeholder="@yourcommunity" value={form.xHandle} onChange={set("xHandle")} />
            </div>
            <div>
              <label className={labelClass}>DISCORD SERVER LINK</label>
              <input className={inputClass} placeholder="discord.gg/yourserver" value={form.discordServer} onChange={set("discordServer")} />
            </div>
            <div className="md:col-span-2">
